# C# Cheatsheet - [Repo Original](https://github.com/jwill9999/C-Sharp-Cheatsheet)

![C#](https://i2.wp.com/programmingwithmosh.com/wp-content/uploads/2015/10/Cover.jpg)
...
## Commented Tasks highlighter [Comentarios que Visual Studio trackea, para mas info apretar aca](https://docs.microsoft.com/es-mx/visualstudio/ide/using-the-task-list?view=vs-2022)
...
```Si no definis nada, por defecto va a ser privada```

```void``` significa que no retorna nada, no hace falta (ni se puede usar) el return. (esto va en el tipo de retorno)
...
### solo para tenerlo de referencia, no recomiendo su uso durante la cursada para evitar confusiones
...
C# es un lenguaje fuertemente tipado. Todas las variables y constantes tienen un tipo, al igual que todas las expresiones que se evalúan como un valor.
...
int c = a + test; //esto no los va a dejar compilar.
```

``` bool no se puede convertir en int```
...
| `!`    | Not. Reverses true or false. | if (!esPar)|
| `&&`     | Logical AND.     |  ```if (valor > 1 && valor <20)``` |
| `||` | Logical OR.      |   ```if (valor > 1 || valor / 2 == 3)``` |
...
### Aclaración: ```Esto es a modo de ejemplo, favor de no utilizar como nombre de variable: i, j , pepito, valor1, etc...```
...
```Por convención las constantes van en mayúscula, pero no es obligatorio para que c# las tome como tal.```
...
```Para que una variable pueda tomar el valor null tiene que ser declarada como nulleable, esto se hace mediante el signo ? como sufijo del tipo de la variable.```
...
## The ability to create classes which inherits certain aspects from parent classes.
...
```Para poder sobreescribir un método (override) necesitamos haber declarado el mismo como virtual o haberlo definido como abstracto dentro de una clase abstracta.```
...
# Referencias SUPER Extra

- [Evolución de app de consola en .NET 6](https://docs.microsoft.com/es-mx/dotnet/core/tutorials/top-level-templates)
